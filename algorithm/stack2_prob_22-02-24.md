...
탐색한 지역을 체크하는 리스트를 사용할 생각을 하지 못했다. 이 방법으로 문제를 해결하려다 보니 재귀를 사용했음에도 코드가 매우 길어졌다. 에러를 수정하려다 보니 조건문이 지저분해 졌다. 시작점을 찾는것을 함수로 뺄 수 있었다. 교수님 조언대로 후보지를 저장하는 방식으로 접근했으면 더 깔끔해졌을것 같다.
