...
- Hoisting은 스크립트 실행 시 변수들이 스코프 최상단으로 끌어올려지는 것을 말한다. 최상단으로 올려지지만 let과 const의 경우 초기화되지 않으며 실제 선언되기 전까진 Temporal Dead Zone(TDZ)가 생성되어 참조가 불가능하다. 반대로 var의 경우 참조 가능하며 값은 undefined를 갖는다.
